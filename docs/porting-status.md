...
| Smooth/mono/LCD raster output | Render-to-bitmap paths and conformance bitmap metadata are present, including public `MODE_LCD_V` exposure for vertical LCD surfaces. | FreeType bitmap bit parity is not closed. Pitch, placement, coverage, LCD filtering, mono packing, and byte-for-byte buffer hashes can still differ. |
| Bitmap fonts and embedded bitmaps | BDF, PCF, FNT, and selected embedded bitmap table support exist, including PPEM-aware sbix, CBLC/CBDT, and EBLC/EBDT strike selection plus payload extraction for common bitmap and PNG data formats. | Bitmap bit parity against FreeType is still an expected gap. Placement, pixel modes, composite embedded bitmap formats, and packed buffer layout need reference comparisons across real bitmap fonts. |
| CFF/CFF2 | Parser and charstring-related support exist, including CFF1 raw Type1C charset/Encoding/FontBBox metadata exposure, CFF2 variation-related work, DICT operand-stack guards, integer-only operand checks for stack-control operators, CFF1 top-level `endchar` and subroutine-return termination checks, CFF2 EOF contour closure and `endchar` rejection, deprecated `dotsection` no-op handling, and operand stack bounds for CFF1/CFF2 charstrings. | Synthetic coverage is not enough. A licensed CFF/CFF2 corpus is needed to verify dicts, raw Type1C charmap behavior, subrs, FDSelect/FDArray, blends, private dict behavior, stack limits, and outline parity against FreeType. |
...
- CFF Type 2 charstrings now tolerate deprecated `dotsection` as a no-op rather
  than treating it as an unsupported escaped operator.
...
- Embedded bitmap payload extraction now strips the metric and PNG length
  headers for common CBLC/CBDT and EBLC/EBDT bitmap data formats before handing
  image bytes to callers, and sbix plus CBLC/EBLC strike lookup now use the
  current requested PPEM instead of always taking the largest or first matching
  strike.
