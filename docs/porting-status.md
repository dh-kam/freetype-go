...
| Bitmap fonts and embedded bitmaps | BDF, PCF, FNT, and selected embedded bitmap table support exist, including CBLC/CBDT and EBLC/EBDT payload extraction for common bitmap and PNG data formats. | Bitmap bit parity against FreeType is still an expected gap. Strike selection, placement, pixel modes, composite embedded bitmap formats, and packed buffer layout need reference comparisons across real bitmap fonts. |
