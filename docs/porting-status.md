...
- Embedded bitmap payload extraction now strips the metric and PNG length
  ...
  image bytes to callers, and CBLC/EBLC strike lookup now uses the current
  requested PPEM instead of always taking the first matching glyph range.
