...
| TrueType hinting VM | Implemented enough for ongoing hinting work and package tests, including prep execution, CVT scaling/mutation, cvar-before-scaling, and size-reset regression coverage. | FreeType instruction semantics are broad. Remaining VM opcode behavior, graphics-state edge cases, twilight/phantom point behavior, target modes, size/driver state interactions, and rollback/error handling need systematic parity tests. |
...
| Type 1 | PFB/PFA input, eexec decoding, dictionary parsing, StandardEncoding/ISOLatin1Encoding/ExpertEncoding, private hint metadata parsing, AFM/PFM companion metric parsers plus explicit face attachment for widths/left side bearings, AFM kerning in `Shape`, basic charstring outlines/metrics/Subrs/flex/stem hint records/native segment records, charstring operand stack bounds including `callsubr`, native segment inspection/dump/compare records, SEAC outline composition, conservative hint snap scaffolding, an `api.Face` loader path, and Go conformance Type 1 fallback smoke coverage exist. | This is useful but not a parity claim. Companion metrics still require explicit attachment rather than automatic public companion discovery, cubic segments are recorded but the shared outline/raster path still flattens them, hinting is not FreeType-equivalent stem/blue-zone fitting, and full OtherSubrs, Multiple Masters, plus real-corpus differential testing remain unsupported or unproven. |
| WOFF/WOFF2 | WOFF and WOFF2 streams can decode into SFNT/TTC, including transformed `glyf`/`loca`, `hmtx`, collection directory reconstruction, and tests for shared collection `glyf`/`loca` references including composite transformed glyphs. | Needs broader real corpus coverage for collection cases, transformed tables, remaining collection variants, error handling, and comparison of decoded faces against FreeType-loaded equivalents. |
...
- Type 1 companion metrics now cover AFM/PFM parsing, public `ReadAFM`,
  `ReadPFM`, and `ReadCompanionMetrics` `io.Reader` helpers, lookup helpers,
  explicit `Face.SetAFM` and `Face.SetPFM` attachment, AFM
  width/left-side-bearing overrides, PFM fallback widths, and AFM KPX kerning
  during `Shape`.
...
- WOFF2 transformed `glyf`/`loca` reconstruction has focused tests for simple
  transformed glyph streams, malformed substreams, zero-length transformed
  `loca`, collection/shared-outline table reconstruction, and shared transformed
  composite glyphs in WOFF2 collections.
...
- Standalone AFM and PFM companion metric parsers, `ReadAFM`/`ReadPFM`
  `io.Reader` helpers, and lookup helpers.
- Explicit Type 1 face companion attachment through `SetAFM` and `SetPFM`.
...
- Native Type 1 path segment recording, including cubic segments for
  inspection/conformance work, segment utility helpers, and conformance dump
  records. The shared `core.Outline` path still receives flattened line points
  for rendering.
- Basic charstring metrics, including width/side-bearing data used by the
  loader.
- Operand stack and subroutine nesting limits reject malformed or runaway Type 1
  charstrings.
...
- Subr calls used by the current charstring interpreter, including regression
  ...
  still respects the operand stack limit.
...
- Automatic discovery/loading of `.afm` and `.pfm` companion files in public
  loader workflows. Current companion metrics require explicit attachment.
- Native cubic outline/raster preservation; Type 1 cubic segments are recorded
  internally, but rendered outlines are still flattened because the shared
  outline/raster contract does not expose cubic tags.
- FreeType-comparable Type 1 hint application, including stem fitting, blue
  zones, and alignment zone behavior.
- Full OtherSubrs semantics beyond the limited cases currently exercised.
...
- **WOFF2 corpus and collection coverage**: Transformed `glyf`/`loca`,
  collection reconstruction, and shared transformed composite glyphs have
  focused synthetic tests, but real WOFF2 corpus coverage, remaining collection
  variants, and FreeType comparison remain open.
...
- **CFF/CFF2 and Type 1 corpus**: Synthetic tests need to be backed by real
  ...
  encodings, metrics, loader behavior, and malformed inputs. Type 1 also still
  lacks automatic companion-file discovery, Multiple Masters, full OtherSubrs,
  native cubic outline preservation, and hinting/stem-zone parity.
