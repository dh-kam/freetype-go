...
- Type 1 companion metrics now cover AFM/PFM parsing, public `ReadAFM`,
  ...
  overrides, PFM fallback widths, and AFM KPX kerning during `Shape`.
- Type 1 encoding support includes ExpertEncoding, and Type 1 charstring
  ...
  SEAC or another `callothersubr`; OtherSubr 3 has a limited pop-value path.
  This is guarded compatibility work, not full OtherSubrs support.
...
- WOFF2 transformed `glyf`/`loca` reconstruction has focused tests for simple
  ...
  malformed shared-`hmtx` outline metadata, and mixed raw/transformed outline
  dependencies.
...
- TrueType VM regression coverage includes MIRP handling for CVT index `-1` as
  ...
  skips invalid negative point indexes, and glyph-program failure coverage
  checks that outline/CVT mutations are rolled back after an error.
...
- Automatic discovery/loading of `.afm` and `.pfm` companion files for plain
  ...
  stream that implements `FontPathStream`; otherwise companion metrics require
  explicit helper calls after a face is loaded.
...
- **WOFF2 corpus and collection coverage**: Transformed `glyf`/`loca`,
  ...
  dependency guards, including raw/transformed outline mismatch and malformed
  shared-composite rejection, have focused synthetic tests, but real WOFF2
...
- **CFF/CFF2 and Type 1 corpus**: Synthetic tests need to be backed by real
  ...
  beyond the current guarded cases, native cubic outline preservation, and
  hinting/stem-zone parity.
