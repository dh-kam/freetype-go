...
| CFF/CFF2 | Parser and charstring-related support exist, including CFF1 raw Type1C charset/Encoding/FontBBox metadata exposure, CFF2 variation-related work, DICT operand-stack guards, integer-only operand checks for stack-control operators, CFF1 top-level `endchar` and subroutine-return termination checks, and operand stack bounds for CFF1/CFF2 charstrings. | Synthetic coverage is not enough. A licensed CFF/CFF2 corpus is needed to verify dicts, raw Type1C charmap behavior, subrs, FDSelect/FDArray, blends, private dict behavior, stack limits, and outline parity against FreeType. |
