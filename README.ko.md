...
- **검증 범위는 확장 중**: 여러 패키지에 테스트가 있고 `tools/conformance`로 Go dump와 선택적 C FreeType dump를 비교할 수 있습니다. 현재 비트 단위 렌더링 동일성이나 전체 사양 지원을 보장하지는 않습니다.
- **FreeType 유사 API 개념**: `api`는 load flag 이름, render mode, 선택적 glyph slot metrics 연결점을 제공합니다. 각 드라이버는 일부만 구현할 수 있고, 남은 의미 차이는 conformance에서 추적합니다.
...
func main() {
	f, _ := os.Open("font.ttf") // TTF, OTF, TTC/OTC, WOFF, WOFF2
	defer f.Close()
