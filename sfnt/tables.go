...
func (t PostTable) GlyphName(glyphIndex int) (string, bool) {
	if glyphIndex < 0 {
		return "", false
	}
	switch t.Version {
	case 0x00010000:
		if glyphIndex >= len(macintoshPostGlyphNames) {
			return "", false
		}
		name := macintoshPostGlyphNames[glyphIndex]
		return name, name != ""
	case 0x00020000:
		if glyphIndex >= len(t.GlyphNameID) {
			return "", false
		}
		nameID := int(t.GlyphNameID[glyphIndex])
		if nameID < len(macintoshPostGlyphNames) {
			name := macintoshPostGlyphNames[nameID]
			return name, name != ""
		}
		customIndex := nameID - len(macintoshPostGlyphNames)
		if customIndex < 0 || customIndex >= len(t.Names) {
			return "", false
		}
		name := t.Names[customIndex]
		return name, name != ""
	default:
		return "", false
	}
...
var macintoshPostGlyphNames = [...]string{
